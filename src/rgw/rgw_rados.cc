...
int RGWRados::init_complete()
{
  ...
  if (ret < 0 )
    return ret;
  bl_d = new RGWBL();
  bl_d->initialize(cct, this);
  bl_d->start_processor();
...
{
  const char *bl_pool = zone.bl_pool.name.c_str();
  int r = rados->ioctx_create(bl_pool, bl_pool_ctx);
  if (r == -ENOENT) {
    r = rados->pool_create(bl_pool);
    if (r == -EEXIST)
      r = 0;
    if (r < 0)
      return r;

    r = rados->ioctx_create(bl_pool, bl_pool_ctx);
  }
...
int RGWRados::open_log_pool_ctx()
{
  ...
  int r = rados->ioctx_create(log_pool, log_pool_ctx);
  if (r == -ENOENT) {
    r = rados->pool_create(log_pool);
    if (r == -EEXIST)
      r = 0;
    if (r < 0)
      return r;

    r = rados->ioctx_create(log_pool, log_pool_ctx);
  }
}
